...
axis_list = tuple([dim for (i, dim) in enumerate(attr_axis_list)])
print(axis_list)

def compute_reducemean(X, *indices):
    attr_axis = 
    print(indices)

Z = te.compute(axis_list, 
               lambda *indices:_compute_div(X, *indices), 
               name="T_compute_reducemean")
