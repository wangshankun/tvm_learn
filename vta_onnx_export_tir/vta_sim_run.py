...
from tvm.contrib import graph_executor, utils, graph_runtime
...
#graph_json = intrp.get_executor_config()
#print(graph_json)

graph = intrp.get_executor_config()

so_name = "resnet18_onnx.so"
temp = utils.tempdir()
path_so = temp.relpath(so_name)
intrp.export_library(path_so)

print("resnet18_onnx upload file: ", path_so)
remote.upload(path_so)
run_lib = remote.load_module(so_name)

run_mod = graph_runtime.create(graph, run_lib, ctx)
run_mod.set_input(**params)

data = np.random.rand(env.BATCH, 3, 224, 224).astype("float32")
run_mod.set_input('data', tvm.nd.array(data))

#run_mod.run()
#res = run_mod.get_output(0).asnumpy()

if env.TARGET in ["sim", "tsim"]:
        simulator.clear_stats()

run_timer = run_mod.module.time_evaluator("run", ctx, number=1, repeat=1)
